...
class Connection(ConnectionBase):
    ...
    def _build_command(self, binary: str, subsystem: str, *other_args: bytes | str) -> list[bytes]:
        ...
        #
        # Additionally, ansible wraps some -o option values in embedded double-quotes (e.g.
        # IdentityFile="<path>", User="<name>") so ssh's POSIX config parser tolerates spaces. On
        # Windows those quotes survive subprocess list2cmdline as \"...\" and corrupt ssh.exe's own
        # command-line tokenization, so the remote command leaks in as a bogus -c cipher argument
        # ("Unknown cipher type ..."). Each value is already a single argv element, so the wrapping
        # quotes are unnecessary here; strip them on Windows.
        if os.name == 'nt':
            filtered: list[bytes] = []
            skip_next = False
            for i, arg in enumerate(b_command):
                ...
                if arg.lower().startswith((b'-ocontrolmaster', b'-ocontrolpersist', b'-ocontrolpath')):
                    continue
                eq = arg.find(b'=')
                if eq > 0 and arg[eq + 1:eq + 2] == b'"' and arg.endswith(b'"'):
