...
(日本語). Section letters refer to PORTING.md §3. Base: ansible-core `2.22.0.dev0`.
...
| `plugins/connection/ssh.py` | Guard `fcntl`/`pty`; threaded pipe reader/selector for non-blocking I/O; strip `ControlMaster`/`ControlPersist`/`ControlPath` on Windows | A, G |
