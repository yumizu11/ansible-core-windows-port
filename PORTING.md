...
Base version: ansible-core `2.21.0` (stable-2.21). A flat per-file list is in
...
* **`_build_command` also strips the embedded double-quotes from `-o` option values (e.g.
  `IdentityFile="…"`, `User="…"`) on `os.name == 'nt'`.** ansible wraps these values in `"…"` so ssh's
  POSIX config parser tolerates spaces; on Windows the quotes survive `subprocess` `list2cmdline` as
  `\"…\"` and corrupt `ssh.exe`'s own command-line tokenization, which then leaks the remote command
  in as a bogus `-c` cipher (`Unknown cipher type 'echo …'`). Each value is already a single argv
