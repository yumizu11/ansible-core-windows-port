...
ベースバージョン: ansible-core `2.21.0`（stable-2.21）。ファイル別の一覧は
...
* **`_build_command` は `os.name == 'nt'` で `ControlMaster`/`ControlPersist`/`ControlPath` の `-o`
  ...
  接続失敗します。
* **`_build_command` は `os.name == 'nt'` で `-o` 値（`IdentityFile="…"`／`User="…"` など）の埋め込み
  二重引用符も除去。** ansible は空白対策で値を `"…"` で囲みますが、Windows ではこの引用符が
  `subprocess` の `list2cmdline` を通って `\"…\"` となり、`ssh.exe` 自身のコマンドライン解析を壊し、
